...
{
    // Convert POLI:// to POLI:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("POLI://"))
    {
        uri.replace(0, 12, "POLI:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "POLI.lnk";
}
...
{
    return GetAutostartDir() / "POLI.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("POLI-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  POLI-qt [" + tr("command-line options") + "]                     " + "\n";
